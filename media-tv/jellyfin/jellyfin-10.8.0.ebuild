...
RDEPEND="
	...
	dev-libs/icu
	dev-util/lttng-ust
	>=media-video/ffmpeg-4.0[encode,fdk,mp3,opus,theora,vorbis,vpx,webp,x264,xvid,x265?,vaapi?,v4l?,libass?]
	sys-libs/glibc"
DEPEND="${RDEPEND}"
...
QA_PREBUILT="
	${PROG_DIR}/createdump
	${PROG_DIR}/jellyfin
	${PROG_DIR}/libcoreclrtraceptprovider.so
	${PROG_DIR}/libSystem.*.so
	${PROG_DIR}/libmscordbi.so
	${PROG_DIR}/libmscordaccore.so
	${PROG_DIR}/libcoreclr.so
	${PROG_DIR}/libclrjit.so
	${PROG_DIR}/libdbgshim.so
